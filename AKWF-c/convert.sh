...
echo -e "/* $basefile 256 samples" >> "$file.h"
...
gnuplot -p -e "set term png size 1920,960; set output \"$file.png\"; plot \"$file\" notitle with lines;"
echo -e "*/\n\n" >> "$file.h"
rm "$file.dat"
rm "$file"
...
echo "const uint16_t "$basefile" [] = {" >> "$file.h"
